...
---| "ENUM_ActionFailCause_NoGuildRenameMark" - гильдия не помечена, как безымянная (требующая переименования)
---| "ENUM_ActionFailCause_NoGuild" - аватар не является членом гильдии
---| "ENUM_ActionFailCause_CCItemsProhibited" - аватар пытается сменить класс, но у него есть предметы, которые несовместимы со сменой класса
---| "ENUM_ActionFailCause_GoalIsNotCompleted" - Цель не выполнена
---| "ENUM_ActionFailCause_RuleIsNotActive" - Правило не выполнено
---| "ENUM_ActionFailCause_MetaItemNotAppear" - Метапредмет не появлялся
---| "ENUM_ActionFailCause_AvatarNotUsedTeleport" - Аватар не использовал телепорт
---| "ENUM_ActionFailCause_NotLookingForGroup"

---@alias ENUM_ACTION_RESULT_SPECIAL
